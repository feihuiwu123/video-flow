...
def compose_scene(
    ...
    """Render one shot: solid-colour background + audio + optional subtitles.

    Args:
        subtitle_start_offset: Seconds to subtract from subtitle timestamps
            so a shot-local copy of the ASS renders correctly. When supplying
            a full-video ASS, keep at 0 and rely on the concat stage.
    """
    _ensure_ffmpeg()
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # lavfi colour source generates a static background at the target resolution.
